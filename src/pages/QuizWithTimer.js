...
// const QuizWithTimer = ({ questions, answers, setAnswers, setFeedbackMessage,setFinalScore,setSubmitted,addPointsToBackend,
//   setResults ,speakLine }) => {
...
//   return () => clearInterval(timer);
// }, [currentIndex, quizCompleted, optionsList]);

// const submitQuizTimer = async () =>{
...
//        setTimeout(() =>    submitQuizTimer(), 500);
//        // submitQuiz();
//       speakLine &&
//         speakLine(
...
//       [questionId]: value,
//     }));
//     setTimeout(() => handleNext(), 500); // short delay before moving next
...
//       <div style={{ marginTop: 25 }}>
//         <Progress
//           percent={(currentIndex / questions.length) * 100}
//           showInfo={false}
...
const QuizWithTimer = ({
  ...
  const submitQuizTimer = async (finalAnswers = answers) => {
    ...
    try {
      const res = await api.post("/quiz/check", {
        answers: questions.map((q) => ({ id: q.id, answer: finalAnswers[q.id] })),
      });
    ...
    >
      ...
      <div style={{ marginTop: 25 }}>
        <Progress
          ...
          style={{ borderRadius: 10 }}
        />
        <Text style={{ fontSize: 24, color: "red" }}>Time Left: {timeLeft}s</Text>
      </div>
    </div>
  );
};
